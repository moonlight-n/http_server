...
http_response* _about_GET(http_request *request)
{
    http_response *response = calloc(1, sizeof(http_response));
    response->body = calloc(1, MAX_BODY_SIZE);
    
    int result = read_file_to_buf("./pages/about.html", response->body);
    if (result == -1)
    {
...
http_response* _about_HEAD(http_request *request)
{
    http_response *response = calloc(1, sizeof(http_response));
    response->body = calloc(1, 1);
    
    char static_buf[MAX_BODY_SIZE];
    int result = read_file_to_buf("./pages/about.html", static_buf);
    if (result == -1)
    {
        error_processing(404, response);
        return response;
